...
router.get("/search/:id", async (req, res) => {
    ...
    const filterTrack = 
        {"track_title": {$regex: searchRequest, $options: "i"}} //Search by titles or album
    const filterAlbum = 
        {"album_title": {$regex: searchRequest, $options: "i"}}
    const select = {track_id: 1, album_title: 1, artist_name: 1, //Pull desired fields from collection
        track_duration: 1, track_title: 1, _id: 0};

    const data = 
        await track.find({$or: [filterTrack, filterAlbum]}, select).limit(8); 
